...
export default async function CarDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const statusColor = {
    Tersedia: "bg-emerald-500/80 text-white",
    Booked: "bg-orange-400/80 text-white",
    Terjual: "bg-slate-400/80 text-white",
  };
          ...
          <div className="glass rounded-3xl overflow-hidden">
            <div className="relative aspect-[16/10]">
              <Image
                ...
                unoptimized
              />
              <span className={`absolute top-4 left-4 px-3 py-1 rounded-full font-[family-name:var(--font-inter)] text-[10px] uppercase tracking-widest ${statusColor[car.status]}`}>
                {car.status}
              </span>
            </div>
          </div>
