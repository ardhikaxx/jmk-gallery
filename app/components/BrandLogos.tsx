      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-10 relative z-10">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-4 md:gap-5">
          {brands.map((brand) => (
            ...
            >
              <div className={`${brand.large ? "w-32 h-32 sm:w-36 sm:h-36" : "w-16 h-16 sm:w-20 sm:h-20"} flex items-center justify-center transition-transform duration-300 group-hover:scale-110`}>
                <Image
                  src={brand.logoUrl}
                  alt={`${brand.name} logo`}
                  width={brand.large ? 144 : 80}
                  height={brand.large ? 144 : 80}
